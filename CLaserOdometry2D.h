...
protected:
  ...
  // Internal Data
  std::vector<Eigen::MatrixXf> range;//储存对range_wf进行处理后的数据 具体算法位于createImagePyramid()函数中 为双边滤波和高斯金字塔
  std::vector<Eigen::MatrixXf> range_old;//储存上一次range的数据
  std::vector<Eigen::MatrixXf> range_inter;//储存R(α) 取值为old和warped的平均值 后缀为inter的下同
  std::vector<Eigen::MatrixXf> range_warped;//储存对公式18的结果进行处理后的数据
  std::vector<Eigen::MatrixXf> xx;//点的x坐标
  std::vector<Eigen::MatrixXf> xx_inter;
  std::vector<Eigen::MatrixXf> xx_old;
  std::vector<Eigen::MatrixXf> xx_warped;
  std::vector<Eigen::MatrixXf> yy;//点的y坐标
  std::vector<Eigen::MatrixXf> yy_inter;
  std::vector<Eigen::MatrixXf> yy_old;
  std::vector<Eigen::MatrixXf> yy_warped;
  std::vector<Eigen::MatrixXf> transformations;//存储每一次的变换矩阵

  Eigen::MatrixXf range_wf;//储存原始激光雷达数据
  Eigen::MatrixXf dtita;//R对α的偏导
  Eigen::MatrixXf dt;//R对t的偏导
  Eigen::MatrixXf rtita;//储存论文公式19所需的d(α)值
  Eigen::MatrixXf normx, normy, norm_ang;
  Eigen::MatrixXf weights;//存储公式14计算的结果
  Eigen::MatrixXi null;
  ...
  MatrixS31 kai_loc_;//储存x方向速度, y方向速度和角度这三个值
  MatrixS31 kai_loc_old_;
  MatrixS31 kai_loc_level_;//储存最小二乘法解出的结果 和变量Var值相同
