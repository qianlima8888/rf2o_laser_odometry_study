...
void CLaserOdometry2D::setLaserPose(const Pose3d& laser_pose)
{
  ...
  laser_pose_on_robot_inv_ = laser_pose_on_robot_.inverse();//矩阵求逆
}
...
bool CLaserOdometry2D::odometryCalculation(const sensor_msgs::LaserScan& scan)
{
  ...
  range_wf = Eigen::Map<const Eigen::MatrixXf>(scan.ranges.data(), width, 1);//获得激光雷达数据
...
//对原始激光雷达数据进行处理后计算获得range 处理算法为双边滤波和高斯金字塔方法
//然后再利用range计算出坐标
void CLaserOdometry2D::createImagePyramid()
{
  ...
  for (unsigned int i = 0; i<pyr_levels; i++)
  {
    ...
    for (unsigned int u = 0; u < cols_i; u++)
    {
      if (range[i](u) > 0.f)
      {
        const float tita = -0.5*fovh + float(u)*fovh/float(cols_i-1); //tita为角度 根据论文公式一变形算出角度
...
void CLaserOdometry2D::calculaterangeDerivativesSurface()
{
  ...
  for (unsigned int u = 1; u < cols_i-1; u++)
    dtita(u) = (rtita(u-1)*(range_inter[image_level](u+1)-
        ...
        range_inter[image_level](u-1)))/(rtita(u)+rtita(u-1));//利用论文公式19
  ...
  //Temporal derivative
  for (unsigned int u = 0; u < cols_i; u++)
    dt(u) = fps*(range_warped[image_level](u) - range_old[image_level](u));//对时间求偏导即为速度 
  ...
  for (unsigned int u = 1; u < cols_i-1; u++)
    if (null(u) == 0)
    {
      ...
      const float w_der = kdt*(dt(u)*dt(u)) +
          ...
          k2d*(std::abs(dtitat) + std::abs(dtita2));//论文公式14的分母根号里面的部分
...
void CLaserOdometry2D::performWarping()
{
  ...
  for (unsigned int j = 0; j<cols_i; j++)
  {
    if (range[image_level](j) > 0.f)
    {
      ...
      //Calculate warping
      const float uwarp = kdtita*(tita_w + 0.5*fovh);//公式18
...
{     
  ...
  Eigen::Matrix3f acu_trans;//储存所有变换矩阵的乘积
  acu_trans.setIdentity();
  ...
  //Filter speed
  //论文公式21
  const float cf = 15e3f*std::exp(-float(int(level))),//ke
              df = 0.05f*std::exp(-float(int(level)));//kl

  Eigen::Matrix<float,3,1> kai_b_fil;
  for (unsigned int i=0; i<3; i++)
  {
    kai_b_fil(i) = (kai_b(i) + (cf*eigensolver.eigenvalues()(i,0) + df)*kai_b_old(i))/(1.f + cf*eigensolver.eigenvalues()(i,0) + df);//论文公式20
...
void CLaserOdometry2D::PoseUpdate()
{
  ...
  //--------------------------------------------------------
  kai_loc_(0) = fps*acu_trans(0,2);//y方向速度
  kai_loc_(1) = fps*acu_trans(1,2);//x方向速度
  ...
  float phi = rf2o::getYaw(laser_pose_.rotation());//角度
  ...
  kai_loc_old_(0) =  kai_abs_(0)*std::cos(phi) + kai_abs_(1)*std::sin(phi);//利用公式5,6计算xy方向速度
